...
const staticCubemapComponent = {
  ...
  init() {
    ...
    mesh ? applyEnvMap(mesh, data.materials, this.texture, data.reflectivity) : this.el.sceneEl.addEventListener('realityready', () => {
      applyEnvMap(this.el.getObject3D('mesh'), data.materials, this.texture, data.reflectivity)
    })
  },
  update(oldData) {
    const {data} = this
...
export default staticCubemapComponent
